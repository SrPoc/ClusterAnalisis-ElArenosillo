# -*- coding: utf-8 -*-
"""
Created on Thu Mar  9 11:34:58 2023

@author: Pablo

Script which takes ERA5 data and selected dates to plot composites
"""
import numpy as np
import pandas as pd
...
ncdata = xr.open_dataset(
    'F:/Tesis/Data/Global Models/ECMWF/ERA5/ERA5_AgSepOct_Europe_Geopotential.nc')
...
levels = np.arange(4950, 6300, 50)

fig, ax = plt.subplots(figsize=(12, 12), dpi=300, )
plt.suptitle(f'{var_names["long_name"]}', fontsize=20, fontweight="bold", x=0.3, y=1)
for nclust in clust:
    #Data corrresponding to nclust:
    composite_days_ncluster = ncdata_intersection.sel(level=level).isel(time=(ClustWDSelec_final['0'] == nclust)).mean(dim='time')
    ...
    im = ax.contourf(nlon, nlat, composite_days_ncluster[ncvariables[0]].values, levels=levels, fontsize=10, cmap=cmap)
    im2 = ax.contour(nlon, nlat, composite_days_ncluster[ncvariables[0]].values, levels=levels,
                      transform=ccrs.PlateCarree(), linewidths =0.4)
    cbar = fig.colorbar(im, ax=ax, orientation='vertical', shrink=0.6, aspect=20, pad=0.05, extend='both')
    cbar.set_label(var_names['units'], fontsize=12, labelpad=10)
